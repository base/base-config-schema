    ...
    if (utils.isObject(collections)) {
      for (var key in collections) {
        ...
        if (typeof collection === 'string' || Array.isArray(collection)) {
          ...
          var type = utils.typeOf(collection);
          var msg = `expected views to be an object, string or array, received: "${type}"`;
          throw new TypeError(msg);
        }
      }
    } else {
      var type = utils.typeOf(collections);
      var msg = `expected collections to be an object, received: "${type}"`;
      throw new TypeError(msg);
    }
