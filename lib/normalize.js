...
function normalize(val, key, config, schema) {
  debug.schema('normalizing field %s: %s', key, util.format(val, null, 2));
  return normalize[type(val)](val, key, config, schema);
}
...
normalize.string = function(str, key, config, schema) {
  var val = normalize.module(str, key, config, schema);

  var obj = {}, alias;
  if (util.isObject(val) && typeof val.pipe === 'function') {
...
normalize.object = function(obj, key, config, schema) {
  ...
  for (var prop in obj) {
    ...
    if (utils.isObject(val)) {
      opts = val;
      val = prop;
      prop = toAlias(prop, key);
    }

    var ele = normalize(val, key, config, schema);
    prop = toAlias(prop, key);
...
normalize.array = function(val, key, config, schema) {
  var len = val.length;
  var idx = -1;
  var obj = {};
  while (++idx < len) {
    obj = utils.merge({}, obj, normalize(val[idx], key, config, schema));
  }
  return obj;
};
